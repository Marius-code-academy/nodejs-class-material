...
// select * from products order by id asc limit 5 offset 10
export async function getProductsPaginate(req, res) {
  const { size, page } = req.query;
  const products = await DB.query(`select * from products order by id asc limit ${size} offset ${(page - 1) * size}`);
